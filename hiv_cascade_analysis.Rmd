...
treat_cascade_country_list_tidy <- map(treat_cascade_country_list, tidy_treat)
names(treat_cascade_country_list_tidy) <- map_chr(names(treat_cascade_country_list_tidy), paste0, "_tidy")
...
test_treat_cascade_country_list_tidy <- map(test_treat_cascade_country_list, tidy_test_treat)
names(treat_cascade_country_list_tidy) <- map_chr(names(treat_cascade_country_list_tidy), paste0, "_tidy")


# join datasets to create full cascade datasets for country datasets
full_cascade_all <- full_join(treat_cascade_all, 
                              test_treat_cascade_all, 
                              by = c("Country", "age", "sex"))

full_cascade_m_adult <- full_join(treat_cascade_m_adult, 
                                  test_treat_cascade_m_adult, 
                                  by = c("Country", "age", "sex"))

full_cascade_f_adult <- full_join(treat_cascade_f_adult, 
                                  test_treat_cascade_f_adult, 
                                  by = c("Country", "age", "sex"))

# union country datasets to include sex and age data in a single dataset

full_cascade_country_tidy <- tidy_composite(full_cascade_country)

full_cascade_country <- union(full_cascade_all,
                                 full_cascade_m_adult) %>%
                        union(full_cascade_f_adult)
```
